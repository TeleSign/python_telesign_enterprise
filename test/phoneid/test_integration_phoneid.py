...
def test_phone_id_path(phoneid):
    ...
    assert (
        phone_id_response.ok
    ), f"Failed to get PhoneID Path for {phone_id_response.json}"
    assert (
        phone_id_response.json.get("status").get("description")
...
def test_phone_id_payload(phoneid):
    ...
    assert (
        phone_id_response.ok
    ), f"Failed to get PhoneID Body for {phone_id_response.json}"
    assert (
        phone_id_response.json.get("status").get("description")
        == "Transaction successfully completed"
    )
