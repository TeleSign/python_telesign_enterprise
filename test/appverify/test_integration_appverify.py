...
def appverify():
    if not CUSTOMER_ID or not API_KEY:
        pytest.skip("CUSTOMER_ID and API_KEY environment variables are required for integration tests.")
...
def test_initiate_call(appverify):
    ...
    assert initiate_response.ok, f"Failed to initiate app verify call: {initiate_response.json}"
...
def test_status(appverify):
    ...
    assert "reference_id" in status_response.json and status_response.json["reference_id"] == reference_id
